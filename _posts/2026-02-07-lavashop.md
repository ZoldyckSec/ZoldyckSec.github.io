...
![Desktop View](/assets/img/lavashop/weblava.png){: width="972" height="589" }
...
```
> Programming lenguages: `PHP` (Identified by Wappalyzer).
...
With an initial foothold established as the debian user, the next objective is to elevate privileges to root.

#### Local Enumeration: Sudo Permissions

A standard first step in local enumeration is checking if the current user has any sudo privileges configured, which might allow executing certain commands as the administrator.

```markdown
debian@Thehackerslabs-LavaShop:/$ sudo -l
[sudo] contraseña para debian: 
Sorry, user debian may not run sudo on Thehackerslabs-LavaShop.
debian@Thehackerslabs-LavaShop:/$
```

> Since the user debian is not allowed to run sudo, I had to look for other escalation vectors.

#### Enumeration of Environment Variables

Misconfigured systems or poorly deployed environments often leak sensitive information within environment variables. To check for this, I executed the env command.

```bash
debian@Thehackerslabs-LavaShop:/$ env
SHELL=/bin/bash
...
- Critical Finding: `ROOT_PASS=lalocadelaslamparas`

#### Root Compromise

Armed with the leaked administrator password, I simply switched the current user context to root using the su command.

```bash
debian@Thehackerslabs-LavaShop:/$ su root
Password: 
